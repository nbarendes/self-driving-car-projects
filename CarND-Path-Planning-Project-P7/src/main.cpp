    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //fill up the rest of our path planner after filling it with previous point 
          for (int i = 1; i<= 50 - previous_path_x.size(); i++)
          {
            double N = (target_dist/(.02*ref_vel/2.24)); //2.24 for the conversion miles per hour -> meter_per_second
